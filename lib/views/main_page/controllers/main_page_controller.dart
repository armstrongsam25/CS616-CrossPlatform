...
@override
  ...
  Future<void> getUsercases() async{
    ...
    busy=true; 
   Status result = await ApiServices.to.getUserCaseList(MainController.to.user_id);
      status=result.code;
      if(result.code==STATUS_CODES.QUERY_SUCCESSFULL.code){
       ...
       for(int i=0; i < result.data.length;i++){
          ...
          developer.log(result.data[i].toJson().toString(), name: 'MainPageController');
       }
      }else{
 ...
 /*
  Future<void> updateList(UserCase userCase) async{

    busy=true;  
    caseList.append(userCase);
    caseList.refresh();
    busy=false; 
